...
IF correlation structure is **too complex**, **statistical power (power of finding a treatment effect) and efficiency suffer**.  
...
# Model 1 - default  
This one assumes the correlation to be 0. 
...
Remember we are using lme because it allow us to set correlation matrices. 
...
Correlation at the deepest level in the hierarchy (DAP) --> Within group residuals.
...
```{r}
# lag 1 correrelation is high because is the correlation with itsel. 
...
Significant autocorrelation in 1 & 2
...
For that, we are going to use the function `anova()` from the `nlme` package (specifyc to lme objects). 
...
AIC and BIC will only be lower when the extra parameters calculated in a more-complex model explain relatively more information than they consumed dfs (explaining very little variability, useless variables). 
...
# Model 3 - General  
Each pair of lag, can have their own rho/correlation value. More flexible. 
```{r}
mod3_gen <- lme(
...
corCAR1 --> Only one that incorporates different intervals. DIFFERENT INTERVALS. 
```{r}
mod4_ar1 <- lme(
  yield_kgha ~ krate_kgha*nrate_kgha*fdap,
  random = ~ 1|block/krate_kgha/nrate_kgha,
...
## intervals  
If lower boundary negative and upper positive , not significantly different from zero. 

If both either negative or positive 
```{r}
...
- Correlation and semivariogram opposite concepts. 
- Here we give an estimate of the range --> Point after which semivariogram plateous out. Where is the break point of time or distances where  is considered a correlation 
...
Compare dap between krate and nrate
This will impact how means are compared --> fdap|krate_kgha:nrate_kgha

This is what we specify based on our research question.
```{r}
...
```
REMEMBER: Impact of repeted measures relies in the ERROR!
